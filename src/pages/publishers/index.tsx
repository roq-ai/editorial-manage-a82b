...
import { getPublishers, deletePublisherById } from 'apiSdk/publishers';
...
export function PublisherListPage(props: PublisherListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PublisherInterface>>(
    () => `/publishers?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      ...
      <Flex justifyContent="space-between" mb={4}>
        ...
        {hasAccess('publisher', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/publishers/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
