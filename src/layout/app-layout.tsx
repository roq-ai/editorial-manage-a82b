...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['system-administrator', 'author', 'reviewer', 'editor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PublisherInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/publishers` : null),
    () => getPublishers({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Publishers',
      path: isTenantUser ? `/publishers/view/${data?.[0]?.id}` : '/publishers',
      entity: 'publisher',
